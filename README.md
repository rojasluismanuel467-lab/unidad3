...
| 4 | **Servicio seguro y con costo controlado** | Deploy con `--no-allow-unauthenticated` + service account dedicada (`u5-g02-sa-20260914`) con solo `roles/storage.objectViewer` + `--min-instances=0` (paga solo por segundo activo) + `--max-instances=1` (techo de costo). |
